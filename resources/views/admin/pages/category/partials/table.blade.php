<table class="table align-middle table-row-dashed fs-6 gy-5">
    ...
    <tr class="text-start text-muted fw-bolder fs-7 gs-0">
        ...
        <th class="min-w-125px">Product Count</th>
        <th class="min-w-125px">Post Count</th>
        <th class="min-w-125px">Created Date</th>
        <th class="text-end min-w-100px"></th>
    </tr>
    </thead>
    <tbody class="text-gray-600 fw-bold">
    @foreach ($categories as $uIndex => $category)
        <tr>
            <td class="d-flex align-items-center">
                <div class="symbol symbol-circle symbol-50px overflow-hidden me-3">
                    <a href="#">
                        <div
                            class="symbol-label fs-3 bg-light">
                            <img class="h-60px w-60px rounded" src="{{ url($category->first_image) }}" alt="">
                        </div>
                    </a>
                </div>
                <div class="d-flex flex-column">
                    <a href="#"
                       class="text-gray-800 text-hover-primary mb-1">{{ $category->translate('en')->name }}</a>
                </div>
            </td>
