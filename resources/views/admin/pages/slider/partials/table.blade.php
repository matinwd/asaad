<table class="table align-middle table-row-dashed fs-6 gy-5">
    ...
    @foreach ($sliders as $uIndex => $slider)
        <tr>
            <td class="d-flex align-items-center">
                <div class="symbol symbol-circle symbol-50px overflow-hidden me-3">
                    <a href="#">
                        <div
                            class="symbol-label fs-3 bg-light-{{ $slider->avatar_color }} text-{{ $slider->avatar_color }}">
                            <img class="h-60px w-60px rounded" src="{{ url($slider->first_image) }}" alt="">
                        </div>
                    </a>
                </div>
                <div class="d-flex flex-column">
                    <a href="#"
                       class="text-gray-800 text-hover-primary mb-1">{{ $slider->translate('en')->name }}</a>
                </div>
            </td>
            <td>
                <div class="badge {{$slider->visibility == 1 ? 'badge-light-success' : 'badge-light-info'}} fw-bolder">
                    {{ $slider->visibility_status_text }}
                </div>
            </td>
            <td>
            ...
            <td class="text-end">
                ...
                <div
                    ...
                    <div class="menu-item px-3">
                        <form method="POST" action="{{ route('admin.sliders.hide',$slider->id) }}">
                            @csrf
                            @method('put')
                            <button type="submit" href="javascript:" class="btn btn-sm menu-link px-3">
                                {{ $slider->visibility == '1' ? 'Hide it' : 'Visible' }}
                            </button>
                        </form>
                    </div>
                </div>
