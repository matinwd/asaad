...
<body id="kt_body" style="background-image: url({{ asset('assets/media/patterns/header-bg-dark.png') }})"
      class="dark-mode header-fixed header-tablet-and-mobile-fixed toolbar-enabled">
            ...
            <div class="toolbar py-5 py-lg-15" id="kt_toolbar">
                <div id="kt_toolbar_container" class="container-xxl d-flex flex-stack flex-wrap">
                    <div class="page-title d-flex flex-column me-3">
                        <h1 class="d-flex text-white fw-bolder my-1 fs-3">@yield('title','Dashboard')</h1>
                    </div>
                    @yield('actions')
                </div>
            </div>
...
<script>
    @auth
        window.wallet_balance = parseFloat("{{ auth()->user()->wallet_balance }}");
    @else
